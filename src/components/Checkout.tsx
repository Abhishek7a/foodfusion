...
import { remove, increaseItemQuantity, decreaseItemQuantity, CartItem } from '../Redux/Reduser/Reducer';
            ...
            {cart.cart.length === 0 ? <h1 className='text-red-600 text-center text-xl pb-10'>Your Cart is Empty</h1> : cart.cart.map((item) => {
              return (
                <div className="flex items-center hover:bg-gray-100 -mx-8 px-6 py-5" key={item.idMeal}>
                  ...
                  <span className="text-center w-1/5 font-semibold text-sm">${Number(item.idMeal.slice(2, 4)) * item.quantity}.00</span>
                </div>
              )
            })}
